...
public class EndergateStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 3000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == MagucnylumBlock.block)
								blockCriteria = true;
							if (blockAt.getBlock() == DragonnylimBlock.block)
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("better_end", "endergate"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("endergate"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("better_end:endergate"), configuredFeature);
		}
	}
	@SubscribeEvent
