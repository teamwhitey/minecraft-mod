...
package net.mcreator.betterend.entity;
...
import net.mcreator.betterend.procedures.SneakerNaturalEntitySpawningConditionProcedure;
import net.mcreator.betterend.itemgroup.BetterendstuffItemGroup;
import net.mcreator.betterend.item.DragonchargeItem;
import net.mcreator.betterend.entity.renderer.SneakerRenderer;
import net.mcreator.betterend.BetterEndModElements;

import com.google.common.collect.ImmutableMap;

@BetterEndModElements.ModElement.Tag
public class SneakerEntity extends BetterEndModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new)
			.size(0.6f, 1.7f)).build("sneaker").setRegistryName("sneaker");
	public SneakerEntity(BetterEndModElements instance) {
		super(instance, 378);
		FMLJavaModLoadingContext.get().getModEventBus().register(new SneakerRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -6750055, -65281, new Item.Properties().group(BetterendstuffItemGroup.tab))
				.setRegistryName("sneaker_spawn_egg"));
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("better_end:dragon").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:enddesert").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:enderdesertsplit").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:enderforest").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:enderhills").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:magicforest").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:obsidantowers").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:sun").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:water").equals(event.getName()))
			biomeCriteria = true;
		...
		if (new ResourceLocation("better_end:youngsunforest").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("better_end:youngwaterforest").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(entity, 7, 4, 4));
	}
	...
	public void init(FMLCommonSetupEvent event) {
		EntitySpawnPlacementRegistry.register(entity, EntitySpawnPlacementRegistry.PlacementType.ON_GROUND, Heightmap.Type.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> {
					...
					int y = pos.getY();
					int z = pos.getZ();
					return SneakerNaturalEntitySpawningConditionProcedure.executeProcedure(ImmutableMap.of("world", world));
				});
	}
	...
	public static class CustomEntity extends CreeperEntity {
		...
		protected void dropSpecialItems(DamageSource source, int looting, boolean recentlyHitIn) {
			...
			this.entityDropItem(new ItemStack(DragonchargeItem.block));
		}
